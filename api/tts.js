...
module.exports = function handler(req, res) {
  ...
  const payload = JSON.stringify({ text: text, model_id: 'eleven_turbo_v2', voice_settings: { stability: 0.65, similarity_boost: 0.25, style: 0.15, use_speaker_boost: true } });
  ...
  const r = https.request({ hostname: 'api.elevenlabs.io', path: '/v1/text-to-speech/' + VOICE, method: 'POST', headers: { 'xi-api-key': KEY.trim(), 'Content-Type': 'application/json', 'Accept': 'audio/mpeg', 'Content-Length': Buffer.byteLength(payload) } }, function(resp) {
    ...
    resp.on('end', function(){ res.setHeader('Content-Type','audio/mpeg'); res.setHeader('Cache-Control','s-maxage=86400'); res.send(Buffer.concat(chunks)); });
  });
